...
RUN apt-get update
RUN apt-get install -y wget unzip xsltproc apache2 default-jre joe lsof sudo libxml2-utils && apt-get clean
...
RUN rm /tmp/*.sh
RUN rm /tmp/*.jar
#RUN cd /usr/local/bin && wget -O virtualhost https://raw.githubusercontent.com/RoverWire/virtualhost/master/virtualhost.sh && chmod +x virtualhost && wget -O virtualhost-nginx https://raw.githubusercontent.com/RoverWire/virtualhost/master/virtualhost-nginx.sh && chmod +x virtualhost-nginx
